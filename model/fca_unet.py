...
class UNet(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def __init__(
        ...
        block_out_channels: Tuple[int, ...] = (72, 72, 72, 72),
        layers_per_block: int = 2,
        downsample_padding: int = 1,
        mid_block_scale_factor: float = 1,
        act_fn: str = "silu",
        norm_num_groups: Optional[int] = 12,
        norm_eps: float = 1e-5,
        cross_attention_dim: int = 256,
        attention_head_dim: Union[int, Tuple[int]] = (4, 4, 4, 4),
        ...
        self.fca = MultiSpectralAttentionLayer(
                    ...
                    dct_w=sample_size[1],
                    reduction=16,
                    freq_sel_method='top3',   # 3 divides 72
                )
    ...
    def forward(
        ...
        # 2. pre-process
        sample = sample.reshape((sample.shape[0] * num_frames, -1) + sample.shape[3:]) # t, c, l, w*h
        sample = self.conv_in(sample) # t, l, l, w*h
        ...
        for i, downsample_block in enumerate(self.down_blocks):
            ...
            if(i == 0):
                fca_input = res_samples[0]
                fca_sample = self.fca(fca_input)    
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if is_final_block and fca_sample is not None:
                sample = sample + fca_sample
...
if __name__ == "__main__":
    ...
    x = torch.randn(1, 31, 72, 32, 32).cuda()
    reference_img = torch.randn(1, 12, 1024).cuda()

    with torch.no_grad():
        output = model(x, reference_img)
